(function(global, factory, define) {
  ...
  function getStyleArrayForBlock(block) {
    ...
    var inlineStyles = {
      ...
      CENTER: new Array(text.length),
      RIGHT: new Array(text.length),
      LEFT: new Array(text.length),
      length: text.length
    }
  ...
  function getStylesAtOffset(inlineStyles, offset) {
    ...
    if (inlineStyles.CENTER[offset]) {
      styles.CENTER = true
    }

    if (inlineStyles.RIGHT[offset]) {
      styles.RIGHT = true
    }

    if (inlineStyles.LEFT[offset]) {
      styles.LEFT = true
    }
  ...
  function addInlineStyleMarkup(style, content) {
    ...
    if (style === 'CENTER') {
      return '<span style="display: block; text-align: center;">'.concat(content, '</span>')
    }

    if (style === 'RIGHT') {
      return '<span style="display: block; text-align: right;">'.concat(content, '</span>')
    }
